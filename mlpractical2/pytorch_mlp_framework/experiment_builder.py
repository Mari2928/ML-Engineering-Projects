...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
        ...
        self.optimizer = optim.Adam(self.parameters(), amsgrad=False, lr=1e-2, #added for experiment
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        layer_names = ['input_conv_conv_0',
                       ...
                       'logit_linear_layer_weight']
        
        for name, params in named_parameters:
            
            name = name.rpartition('.')
            name1 = name[0].rpartition('.')[-3].rpartition('.')[0].rpartition('.')[-1]
            name2 = name[0].rpartition('.')[-1]
            ...
            if layer_name in layer_names and layer_name not in layers:    
                layers.append(layer_name) 
                all_grads.append(params.grad.absolute().mean().item())
